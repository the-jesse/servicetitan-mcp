...
5. `src/server.ts` — `createServer()` constructs the `McpServer` and registers all domain tools (`registerCustomerTools`, `registerJobTools`, `registerAppointmentTools`, `registerTechnicianTools`, `registerInvoiceTools`, `registerWebhookTools`) plus the `servicetitan___ping` health check. When adding a new domain, add the corresponding `register<Domain>Tools(server)` call here. Note: many endpoint paths and response shapes inside the domain modules are still placeholders — the wiring is complete, but individual tools are not yet verified against a live tenant.
